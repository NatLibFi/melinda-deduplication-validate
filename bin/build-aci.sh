...
ACI_NAME="deduplication-validate-ubuntu"
ACI_VERSION="1.0.0"
...
$ACBUILD_CMD set-working-directory /opt/melinda-deduplication-validate/app
$ACBUILD_CMD set-exec -- /bin/bash -c '/usr/bin/node index.js 2>&1 | tee -a /opt/melinda-deduplication-validate/logs/melinda-deduplication-validate.log'

$ACBUILD_CMD mount add logs /opt/melinda-deduplication-validate/logs
$ACBUILD_CMD mount add data /opt/melinda-deduplication-validate/data
$ACBUILD_CMD mount add --read-only conf /opt/melinda-deduplication-validate/conf

$ACBUILD_CMD copy aci-build/app /opt/melinda-deduplication-validate/app
...
$ACBUILD_CMD run --engine $ACBUILD_ENGINE --working-dir /opt/melinda-deduplication-validate/app -- /bin/bash -c 'npm install --production'
$ACBUILD_CMD run --engine $ACBUILD_ENGINE --working-dir /opt/melinda-deduplication-validate/app/melinda-deduplication-common -- /bin/bash -c 'npm install --production'
